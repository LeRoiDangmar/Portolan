...
- source_spec: `_bmad-output/specs/spec-Portolan/stories/3-shared-graph-model-package.md`
  summary: FR-12 enumerates three health readings and Swarm can report a fourth — `running > desired`, transiently, during a rolling update — so the story that draws the health mark has no branch for it.
  evidence: `packages/model/src/graph.ts`'s `Health` stores the counted pair faithfully and correctly; `readCount` accepts each field independently, and `{ running: 6, desired: 5 }` round-trips clean. Rejecting it at the wire would refuse a real cluster, so the model is not the place to fix it. The gap is in the PRD's enumeration (FR-12) and lands wherever the mark is read — story 13 or the scene. What would settle it: a product decision on which of blue / amber / red an over-replicated service takes, or a fourth value.
